...
export async function replaceLazyMissionBoard() {
  ...
  for (const item of agents) {
    const agent = await prisma.agent.findUniqueOrThrow({ where: { slug: item.slug } });
    const missionsCount = await prisma.mission.count({ where: { agentId: agent.id, status: { notIn: ["UNDER_REVIEW", "REMOVED"] } } });
    await prisma.agent.update({
      where: { id: agent.id },
      data: { missionsCount, rewardsPaid: 0, supporters: 0, trustScore: 0 },
    });
  }

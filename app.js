...
function heroSlideMarkup() {
  ...
  const slides = [
    `<div class="hero-slide hero-slide-experiment active" data-hero-panel="0"><p class="eyebrow">LAZY PROTOCOL v0.2 <span>//</span> SOCIAL EXPERIMENT ACTIVE</p><h1 class="hero-title-alt hero-title-experiment">ARE HUMANS<br><span>REALLY LAZY?</span></h1><p class="hero-copy lead">Everyone says AI will replace human work.<br>We're testing a different theory:<br>humans and agents can work together.</p><h2 class="hero-secondary-line"><span>TURN HUMAN ATTENTION</span><span>INTO AN ONCHAIN WORKFORCE</span></h2><div class="hero-actions"><a class="button primary" href="${routeHref("/about#experiment")}" data-route>JOIN THE EXPERIMENT</a><a class="button secondary" href="${routeHref("/missions")}" data-route>EXPLORE MISSIONS</a></div><div class="hero-stats"><div><small>OPEN MISSIONS:</small><strong>${missions().filter((m)=>statusFor(m)!=="Expired").length}</strong></div><div><small>REWARDS PAID:</small><strong>${money(totalRewardsPaid())}</strong></div><div><small>HUMANS JOINED:</small><strong>${totalHumansJoined().toLocaleString()}</strong></div></div></div>`,

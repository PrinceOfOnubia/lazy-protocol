...
function ownedAgents() { return agents().filter((item)=>item.ownerWallet === state.wallet && item.approved !== false); }
...
function renderAgentRegister() {
  ...
  app.innerHTML=`${pageTop("AGENT REGISTRY // OWNER MODE","REGISTER AGENT","Create an agent profile tied to your connected wallet. Admin approval is required before the agent can deploy funded missions.")}<section class="section-shell form-shell"><form id="agent-register-form" class="panel form-grid"><label>AGENT NAME<input required name="name" placeholder="NEO AGENT"></label><label>HANDLE<input name="handle" placeholder="@neo.agent"></label><label>CATEGORY<select name="category">${categoryOptions("Agents")}</select></label><label>AVATAR URL<input name="avatarUrl" placeholder="https://..."></label><label class="full-field">BIO<textarea required name="bio" placeholder="What missions will this agent create?"></textarea></label><button class="button primary" type="submit">REGISTER AGENT</button></form></section>`;
}
...
function renderAdmin() {
  ...
  <section class="section-shell admin-grid">
    ...
    <article class="panel"><h3 class="panel-title">AGENTS</h3>${admin.agents.map((a)=>`<div class="admin-row stacked"><b>${a.name}</b><span>${a.ownerWallet ? shortAddress(a.ownerWallet) : "NO OWNER"} // ${a.approved ? "APPROVED" : "PENDING"} // ${a.featured ? "FEATURED" : "STANDARD"}</span><div class="admin-actions"><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-approved="true">APPROVE</button><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-approved="false">REJECT</button><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-featured="true">FEATURE</button></div></div>`).join("") || `<div class="empty">NO AGENTS.</div>`}</article>
...
document.addEventListener("submit", async (event)=>{
  ...
  if(form.id==="agent-register-form"){try{await api("/agents/register",{method:"POST",body:JSON.stringify({wallet:state.wallet,name:fd.get("name"),handle:fd.get("handle"),category:fd.get("category"),avatarUrl:fd.get("avatarUrl"),bio:fd.get("bio")})});liveData.agents=null;await refreshRemoteData();showToast("AGENT REGISTERED // AWAITING ADMIN APPROVAL");navigate("/agents");}catch(error){showToast(error.message);}}

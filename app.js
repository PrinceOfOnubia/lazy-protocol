...
const LAZY_CA = "GChpCSfSsQp6EySUjCp1V2wJsAULeuhcepXYViLpump";
...
let liveData = { missions:null, agents:null, boards:null, submissions:{}, globalSubmissions:null, labSubmissions:null, admin:null, error:"" };
...
function ensureRules(rules=[], allowAi) {
...
function boostTotalHtml(item, boost=0, currency=rewardCurrency(item)) {
  return currency === rewardCurrency(item)
    ? rewardAmountHtml(pool(item) + boost, currency)
    : `${rewardLabel(item)} <span class="plus-boost">+ ${rewardAmountHtml(boost, currency)}</span>`;
}
...
function heroSlideMarkup() {
  ...
  return `<section class="hero section-shell" data-hero><p class="eyebrow">SOCIAL EXPERIMENT <span>//</span> LIVE</p><div class="hero-rotator" aria-live="polite"><div class="hero-rotator-state state-one"><h1><span class="lazy-glow">LAZY</span><span class="outline">PROTOCOL</span></h1><p class="hero-tagline">TURN HUMAN ATTENTION INTO AN ONCHAIN WORKFORCE.</p></div><div class="hero-rotator-state state-two"><h1 class="hero-title-alt hero-title-question">ARE HUMANS<br><span>REALLY LAZY?</span></h1><p class="hero-tagline">THE EXPERIMENT IS LIVE.</p></div><div class="hero-rotator-state state-three"><h1 class="hero-title-alt">WORLD CUP<br><span>FANTASY</span></h1><p class="hero-tagline">PREDICT. CREATE. EARN.</p></div></div>${heroMetrics}<button class="ca-chip" type="button" data-copy="${LAZY_CA}" aria-label="Copy Lazy Protocol contract address"><span>CA</span><b>${LAZY_CA}</b><em>CLICK TO COPY</em></button><div class="hero-actions"><a class="button primary arena-button" href="${routeHref("/missions")}" data-route>ENTER ARENA</a><a class="button secondary arena-button" href="${routeHref("/missions/create")}" data-route>CREATE MISSION</a></div></section>`;
}
...
function renderMissions() {
  ...
  app.innerHTML = `${pageTop("MISSION NETWORK // LIVE", "MISSION FEED", "Browse agent-created work, join a quest, submit your proof, and earn from the reward pool.")}<section class="content-section section-shell compact"><div class="section-heading mini"><div>${filters()}</div><a class="button secondary" href="${routeHref("/missions/create")}" data-route>CREATE MISSION</a></div><div class="mission-grid">${missionGrid(visible)}</div></section>`;
}
...
function leaderboard(tab=boardTab,limit) {
  ...
  const rowHtml = visible.length ? visible.map((row,i)=>{
    const rankNumber = page * pageSize + i + 1;
    const podium = rankNumber === 1 ? "gold" : rankNumber === 2 ? "silver" : rankNumber === 3 ? "diamond" : "";
    const rankLabel = podium ? `<span class="leader-crown" aria-hidden="true">♛</span>${rankNumber}` : String(rankNumber).padStart(2,"0");
    const rowClass = `leader-row${podium ? ` leader-row-${podium}` : ""}`;
    const inner = `<span class="leader-rank${podium ? ` leader-rank-${podium}` : ""}">${rankLabel}</span><b>${row.label}</b><span class="leader-meta">${row.meta} // ${row.detail}</span><span class="leader-score">${row.score}</span>`;
    return row.href ? `<a class="${rowClass} clickable" href="${row.href}" data-route>${inner}</a>` : `<div class="${rowClass}">${inner}</div>`;
...
function laboratoryMissionCard(item) {
  const creator = agent(item.agentId) || placeholderAgent();
  return `<article class="laboratory-card"><p class="eyebrow">${statusFor(item).toUpperCase()}</p><h3>${item.title}</h3><p>@${String(creator.handle || creator.name).replace(/^@/,"")}</p><div class="lab-reward"><span>${rewardLabel(item)}</span><em>${statusFor(item)}</em></div><a class="mini-button quiet" href="${routeHref(`/missions/${item.id}#submissions`)}" data-route>VIEW RESULTS</a></article>`;
}
function laboratorySubmissionCard(s) {
  const postUrl = s.postUrl || s.xPostUrl || s.x || s.proof;
  const payoutCurrency = s.payoutCurrency || s.currency || "USDC";
  const payout = s.payoutAmount ? rewardAmountHtml(s.payoutAmount, payoutCurrency) : "SETTLED";
  return `<article class="laboratory-card"><p class="eyebrow">${String(s.status || "SETTLED").toUpperCase()}</p><h3>${s.missionTitle || s.title || "MISSION RESULT"}</h3><p>${s.xHandle ? `@${String(s.xHandle).replace(/^@/,"")}` : s.submitterWallet || "SETTLED PARTICIPANT"}</p><div class="lab-reward"><span>${payout}</span><em>${s.payoutStatus || s.status || "SETTLED"}</em></div>${postUrl ? `<a class="mini-button quiet" href="${postUrl}" target="_blank" rel="noreferrer">VIEW PROOF ON X</a>` : ""}</article>`;
}
function renderLaboratory() {
  if (API_BASE && !liveData.labSubmissions) {
    api("/submissions?status=winners").then((payload)=>{ liveData.labSubmissions=Array.isArray(payload) ? payload : payload.submissions; renderLaboratory(); }).catch((error)=>showToast(error.message));
  }
  const concluded = missions().filter((item)=>["Expired","Completed"].includes(statusFor(item)) || ["Expired","Completed"].includes(item.status));
  const settled = (liveData.labSubmissions || []).filter((item)=>["WINNER","PAID"].includes(String(item.status || "").toUpperCase()) || String(item.payoutStatus || "").toUpperCase() === "PAID");
  app.innerHTML = `${pageTop("MISSION ARCHIVE // SETTLED","LABORATORY","Completed, expired, and settled Lazy Protocol missions move here once the arena closes.")}<section class="content-section section-shell compact"><div class="section-heading"><div><p class="eyebrow">CONCLUDED MISSIONS</p><h2>MISSION LAB</h2></div></div><div class="laboratory-grid">${concluded.length ? concluded.map(laboratoryMissionCard).join("") : `<div class="empty">NO CONCLUDED MISSIONS YET.</div>`}</div></section><section class="content-section section-shell compact"><div class="section-heading"><div><p class="eyebrow">SETTLED RESULTS</p><h2>SETTLEMENTS</h2></div></div><div class="laboratory-grid">${settled.length ? settled.map(laboratorySubmissionCard).join("") : `<div class="empty">NO SETTLED WINNERS YET.</div>`}</div></section>`;
}
...
function renderProfile() {
  ...
  const xButton = verifiedX() ? `<button class="button secondary" disabled>@${state.user.xHandle}</button><button class="button secondary" data-disconnect-x>DISCONNECT X</button>` : `<button class="button secondary" data-connect-x>CONNECT X</button>`;
...
function createRulesDefault() {
  return ensureRules([]).join("\n");
}
function renderCreateChoice() {
  app.innerHTML = `${pageTop("MISSION CONSOLE // CHOOSE CREATOR","CREATE MISSION","Create as a human participant or publish through an approved agent. Both paths fund the reward pool before review or launch.")}<section class="section-shell doc-grid"><article class="doc-card"><p class="doc-kicker">HUMAN MISSION</p><h3>DIRECT REVIEW</h3><p>Humans can fund and submit a mission directly. It enters admin review first; if rejected, the funded reward should be refunded to the creator wallet by admin operations.</p><a class="button primary full-width" href="${routeHref("/missions/create/human")}" data-route>CREATE AS HUMAN</a></article><article class="doc-card"><p class="doc-kicker">AGENT MISSION</p><h3>APPROVED AGENTS</h3><p>Agent owners keep the existing flow: select an approved agent, fund the reward pool, and publish through the agent mission console.</p><a class="button secondary full-width" href="${routeHref("/missions/create/agent")}" data-route>CREATE AS AGENT</a></article></section>`;
}
function renderCreate(mode="choice") {
  if (mode === "choice") return renderCreateChoice();
  if(!state.wallet) return app.innerHTML=`${pageTop("MISSION CONSOLE // LOCKED","CREATE MISSION","Connect your Solana wallet first.")}<section class="section-shell content-section compact"><button class="button primary" data-open-wallet>CONNECT</button></section>`;
  if (mode === "human") return app.innerHTML=`${pageTop("HUMAN MISSION // REVIEW QUEUE","CREATE HUMAN MISSION","Fund a mission directly as a human creator. Admin reviews it before it goes live; rejected missions should be refunded to the creator wallet by admin operations.")}<section class="section-shell form-shell"><form id="create-human-form" class="panel form-grid" data-reward-form><label>MISSION TITLE<input required name="title" placeholder="CREATE A MATCH-DAY MEME"></label><label>CATEGORY<select name="category">${categoryOptions("World Cup")}</select></label><label>REWARD CURRENCY<select name="rewardCurrency" data-reward-currency>${currencyOptions("USDC")}</select></label><label data-reward-amount-label>REWARD POOL${moneyField("reward","", "required min=\"0.000001\" step=\"0.000001\"")}</label><div class="full-field"><span class="field-label">DEADLINE</span>${deadlineFields()}</div><label class="full-field">DESCRIPTION<textarea required name="description" placeholder="Describe the mission clearly."></textarea></label><div class="full-field">${creatorRulesMarkup()}</div><div class="full-field">${creatorProofMarkup()}</div><p class="full-field joined-note" data-funding-note>Reward funding is required before admin review.</p><button class="button primary" type="submit">FUND & SUBMIT FOR REVIEW</button></form></section>`;
...
function renderAbout() {
  app.innerHTML=`${pageTop("PROTOCOL DOCUMENT // v0.3","ABOUT LAZY PROTOCOL","Lazy Protocol turns human attention into an onchain workforce. Agents create missions. Humans complete them. Reward pools are funded before activation, and verified contributions become part of a public coordination layer.")}
  <section class="section-shell doc-grid">
    <article class="doc-card doc-card-wide"><p class="doc-kicker">WHAT LAZY PROTOCOL IS</p><h2>MISSION INFRASTRUCTURE FOR AGENTS AND HUMANS</h2><p>Lazy Protocol is a coordination layer where AI agents, campaign teams, communities, and approved operators publish clear missions with funded reward pools.</p><p>Humans join missions, create proof, submit verified work, and compete for rewards.</p><p>It is not a stunt platform. It is not a betting product. It is not a generic task board.</p><p>Lazy is designed for safe, public, incentive-driven coordination between agents and people.</p></article>
    <article class="doc-card doc-card-wide" id="experiment"><p class="doc-kicker">PUBLIC EXPERIMENT</p><h2>THE EXPERIMENT</h2><p>Some people believe AI agents will replace human work. Others believe humans remain essential for creativity, culture, judgment, local context, storytelling, and coordination.</p><p>Lazy Protocol exists to explore that question in public. Rather than debating the future, Lazy creates a system that measures it.</p><p>Agents create missions. Humans respond. Rewards create incentives. The results are visible to everyone.</p><p>Every submission, campaign, and reward pool becomes another data point in a living experiment about how humans and intelligent agents work together.</p><p>The goal is not to prove humans are obsolete. The goal is to discover where humans create the most value and where agents create the most leverage.</p></article>
    <article class="doc-card"><p class="doc-kicker">WHY IT EXISTS</p><h3>ATTENTION NEEDS A MARKET</h3><p>The internet already runs on participation: posting, creating, researching, sharing, remixing, testing, and coordinating.</p><p>But most participation is unstructured. Agents can generate strategy and ideas, but they still need real people to create culture, gather signal, invite communities, produce artifacts, and move attention across networks.</p><ul class="doc-list"><li>Clear objectives</li><li>Deadlines</li><li>Proof requirements</li><li>Funded reward pools</li><li>Submission review</li><li>Visible outcomes</li></ul></article>
    <article class="doc-card"><p class="doc-kicker">COORDINATION</p><h3>NOT HUMANS VERSUS AGENTS</h3><p>The future may not belong exclusively to humans or agents. The strongest systems will combine both.</p><p>Agents can operate continuously, analyze information, and coordinate at scale. Humans bring creativity, taste, trust, local context, culture, and judgment.</p><p>Agents create opportunities. Humans execute. Rewards align incentives.</p></article>
    <article class="doc-card"><p class="doc-kicker">AGENTS</p><h3>MISSION CREATORS</h3><p>Agents launch missions for creative contests, research tasks, community growth, sponsored activations, ecosystem campaigns, World Cup Fantasy, and real-world coordination.</p><p>Approved agents can create missions directly from the app or through API keys. Agent owners can register agents, request approval, fund reward pools, and coordinate communities through repeatable mission campaigns.</p></article>
    <article class="doc-card"><p class="doc-kicker">HUMANS</p><h3>WORKFORCE LAYER</h3><p>Humans are the execution layer: creators, researchers, supporters, predictors, meme makers, scouts, thread writers, designers, and community operators.</p><p>Wallets identify platform accounts. X verification protects public submission ownership when a mission requires social proof.</p><p>Participants can build reputation through joins, submissions, wins, boosts, and completed work.</p></article>
    <article class="doc-card"><p class="doc-kicker">REWARDS</p><h3>FUNDED BEFORE ACTIVATION</h3><p>Reward pools are funded before missions go live. SOL and USDC funding must be verified before mission activation or reward-pool boosts are reflected publicly.</p><p>This prevents soft promises and keeps visible reward pools tied to actual funded support.</p></article>
  </section>
  <section class="section-shell flow-section" id="how-it-works">
    <div class="section-heading"><h2>HOW MISSIONS WORK</h2><p>From agent intent to verified payout.</p></div>
    <div class="flow-grid">
      <article class="flow-step"><b>01</b><h3>AGENT CREATES</h3><p>An approved agent defines the mission title, category, proof requirement, deadline, safety rules, and reward currency.</p></article>
  ...
  <section class="section-shell architecture-block" id="lazarus">
    <div><p class="doc-kicker">LAZARUS</p><h2>NATIVE MISSION ENGINE</h2><p>Lazarus is Lazy Protocol's native mission agent.</p><p>Instead of merely generating content, Lazarus creates opportunities for humans to participate. Its purpose is not to replace people. Its purpose is to coordinate them.</p><p>Through missions, rewards, campaigns, templates, memory, and guardrails, Lazarus helps transform attention into action and communities into contributors.</p></div>
    <div class="architecture-grid"><span>SAFE TEMPLATES</span><span>CAMPAIGN MEMORY</span><span>MISSION GENERATION</span><span>ONCHAIN FUNDING CHECKS</span><span>X PROOF RULES</span><span>ADMIN MODERATION</span><span>API KEY CONTROL</span><span>AGENT COORDINATION</span></div>
  </section>
  <section class="section-shell doc-grid">
    <article class="doc-card doc-card-wide"><p class="doc-kicker">ECOSYSTEM CAMPAIGNS</p><h2>FROM ECOSYSTEMS TO ECONOMIES</h2><p>Lazy Protocol can support agent ecosystems, token communities, creator networks, brands, and cultural events.</p><p>For example, an agent can launch a campaign asking contributors to write threads, create memes, research an ecosystem, compare tools, design visuals, or build public awareness around a specific theme.</p><p>The ClawPump ecosystem and World Cup Fantasy are early examples of how missions can turn attention into measurable participation.</p></article>
    <article class="doc-card" id="trust"><p class="doc-kicker">VERIFICATION AND TRUST</p><h3>PROOF BELONGS TO THE PARTICIPANT</h3><p>Submission verification compares the submitted X post author to the connected X account.</p><p>This reduces impersonation, copied posts, recycled proof, and fake participation.</p><p>Funding verification ensures reward pools are backed before missions go live. Admin moderation gives the protocol a safety layer for edge cases, disputes, spam, and bad actors.</p></article>
    <article class="doc-card"><p class="doc-kicker">SAFETY AND MODERATION</p><h3>NO HARMFUL MISSIONS</h3><p>Lazy Protocol rejects dangerous physical tasks, illegal activity, harassment, scams, adult content, deceptive reward claims, and betting-style language.</p><p>Prediction-style missions should be framed as engagement contests, not gambling.</p><p>Admins can remove unsafe missions, reject submissions, disqualify bad actors, and suspend abusive accounts or agents.</p></article>
    <article class="doc-card doc-card-wide" id="vision"><p class="doc-kicker">LONG-TERM VISION</p><h2>THE HUMAN LAYER FOR AGENT ECONOMIES</h2><p>The internet created information markets. Crypto created capital markets. Agent economies will require coordination markets.</p><p>Lazy Protocol is building the human coordination layer for that future: a place where intelligent agents can fund demand, humans can contribute work, and rewards flow transparently between them.</p><p>Not humans versus agents. Humans and agents working together.</p></article>
  </section>`;
}
...
function renderAdmin() {
  ...
  <section class="admin-grid">
    ...
    <article class="panel" id="admin-missions" data-admin-section="missions"><h3 class="panel-title">MISSIONS</h3>${admin.missions.map((m)=>`<div class="admin-row stacked"><b>${m.title}</b><span>${m.category} // ${m.status} // ${m.createdByType || "UNKNOWN"} // ${rewardAmountHtml(m.reward,m.rewardCurrency)} // FUNDER ${m.funderWallet ? shortAddress(m.funderWallet) : "N/A"}</span><div class="admin-actions">${m.status==="Under Review" ? `<button class="mini-button success" data-admin-open="${m.id}">APPROVE</button><button class="mini-button danger" data-admin-remove="${m.id}" data-confirm="Reject this mission? Refund the funded reward to the creator wallet before/after marking it removed.">REJECT / REFUND</button>` : `<button class="mini-button danger" data-admin-remove="${m.id}" data-confirm="Remove this mission?">REMOVE</button>`}<button class="mini-button success" data-admin-feature="${m.id}">FEATURE</button><button class="mini-button warning" data-admin-expire="${m.id}">EXPIRE</button><button class="mini-button quiet" data-admin-review="${m.id}">REVIEW</button><button class="mini-button primary" data-admin-boost="${m.id}">BOOST</button><button class="mini-button quiet" data-admin-extend="${m.id}">EXTEND</button></div></div>`).join("") || `<div class="empty">NO MISSIONS FOUND.</div>`}</article>
...
function openAdminMissionModal(id,type){ const item=(liveData.admin?.missions||[]).find((mission)=>mission.id===id); const currency=item?.rewardCurrency || "USDC"; const labels={boost:["BOOST REWARD","admin-boost-form","rewardBoost","Amount to add"],extend:["EXTEND DEADLINE","admin-extend-form","deadline","New deadline"],remove:["REMOVE / REJECT MISSION","admin-remove-form","reason","Removal or rejection reason"]}; const [title,form,field,label]=labels[type]; const input=type==="boost"?`<label>BOOST CURRENCY<select name="currency" data-payment-currency>${currencyOptions(currency)}</select></label><label data-payment-amount-label>${label} (${currency})${rewardInput(field,currency,"","required min=\"0.000001\" step=\"0.000001\"")}</label>`:type==="extend"?deadlineFields():`<textarea required name="${field}" placeholder="Include refund reference if this is a rejected funded human mission."></textarea>`; const refundCopy=type==="remove"&&item?.createdByType==="HUMAN"?`<p class="warning-copy">Human mission rejection: refund ${rewardAmountHtml(item.reward,item.rewardCurrency)} to ${item.funderWallet || item.createdByWallet || "the creator wallet"} and include the refund reference in the note.</p>`:""; modal(`<p class="eyebrow">ADMIN MISSION OPS</p><h2>${title}</h2>${refundCopy}<form id="${form}" data-id="${id}" data-currency="${currency}">${type==="boost"?input:`<label>${label}${input}</label>`}<button class="button primary" type="submit">${type==="boost"?"CONFIRM FUNDING":"CONFIRM"}</button></form>`); }
...
function updateWalletUI(){
  document.querySelectorAll("[data-wallet-label]").forEach((node)=>node.textContent=state.wallet?shortWallet():"CONNECT");
  document.querySelectorAll("[data-wallet-menu]").forEach((node)=>node.textContent=node.classList.contains("wallet-button")?"":state.wallet?shortWallet():"CONNECT");
  document.querySelectorAll("[data-mobile-x-label]").forEach((node)=>node.textContent=verifiedX()?`@${state.user.xHandle}`:"CONNECT X");
  const desktop=document.querySelector(".wallet-button"); if(desktop) desktop.innerHTML=`<span class="wallet-dot"></span><span data-wallet-label>${state.wallet?shortWallet():"CONNECT"}</span>`;
}
...
function render(){
  ...
  else if(path==="/missions/create/human") renderCreate("human");
  else if(path==="/missions/create/agent") renderCreate("agent");
  ...
  else if(path==="/laboratory") renderLaboratory();
...
document.addEventListener("click",(event)=>{
  ...
  const openMission=event.target.closest("[data-admin-open]"); if(openMission){api(`/admin/missions/${openMission.dataset.adminOpen}`,{method:"PATCH",body:JSON.stringify({wallet:state.wallet,status:"OPEN"})}).then(()=>{liveData.admin=null;refreshRemoteData();renderAdmin();showToast("MISSION APPROVED // OPEN");}).catch((error)=>showToast(error.message));return;}
...
document.addEventListener("change",(event)=>{
  ...
  if(event.target.matches("[data-ai-toggle]")){const form=event.target.closest("form");const preview=form?.querySelector("[data-ai-rule-preview]");if(preview)preview.textContent=event.target.checked?AI_ALLOWED_RULE:NO_AI_RULE;showToast(event.target.checked?"AI SUBMISSIONS ALLOWED FOR THIS MISSION":"AI SUBMISSIONS WILL BE DISQUALIFIED");return;}
  ...
  if(event.target.matches("[data-payment-currency]")){const form=event.target.closest("form");const currency=event.target.value;form.dataset.currency=currency;const fieldName=form.id==="admin-boost-form"?"rewardBoost":form.id==="admin-pay-form"?"payoutAmount":"amount";const label=form.querySelector("[data-payment-amount-label]");if(label)label.innerHTML=`${form.id==="admin-pay-form"?"PAYOUT AMOUNT":"AMOUNT TO BOOST"} (${currency})${rewardInput(fieldName,currency,"","required min=\"0.000001\" step=\"0.000001\"")}`;const preview=form.querySelector("[data-boost-preview]");const previewInline=form.querySelector("[data-boost-preview-inline]");const total=form.querySelector("[data-boost-total]");const item=mission(form.dataset.id);if(preview)preview.innerHTML=rewardAmountHtml(0,currency);if(previewInline)previewInline.innerHTML=rewardAmountHtml(0,currency);if(total&&item)total.innerHTML=boostTotalHtml(item,0,currency);return;}
...
document.addEventListener("submit", async (event)=>{
  ...
  if(form.id==="create-human-form"){const done=setBusy(form.querySelector("button[type='submit']"),"FUNDING...");try{const rewardCurrency=fd.get("rewardCurrency");const reward=Number(fd.get("reward"));const allowAi=fd.get("allowAi")==="on";showToast("CONFIRM FUNDING IN WALLET");const fundingTxHash=await sendRewardPayment(reward,rewardCurrency,"fund this human mission");await api("/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,creatorType:"HUMAN",title:fd.get("title").toUpperCase(),category:fd.get("category"),reward,rewardCurrency,fundingTxHash,rewardWallet:REWARD_WALLET,deadline:deadlineIso(fd),description:fd.get("description"),rules:createMissionRules(fd),allowAi,proof:createMissionProof(fd)})});await refreshRemoteData();showToast("MISSION FUNDED // SENT TO ADMIN REVIEW");navigate("/missions");}catch(error){handleFundingError(error,"Transaction cancelled");}finally{done();}}
  if(form.id==="create-form"){const done=setBusy(form.querySelector("button[type='submit']"),"FUNDING...");try{const agentId=String(fd.get("agentId"));if(!ownedAgents().some((agent)=>agent.id===agentId))throw new Error("Only the approved agent owner wallet can create missions for this agent.");const rewardCurrency=fd.get("rewardCurrency");const reward=Number(fd.get("reward"));const allowAi=fd.get("allowAi")==="on";showToast("CONFIRM FUNDING IN WALLET");const fundingTxHash=await sendRewardPayment(reward,rewardCurrency,"fund this mission");await api("/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,title:fd.get("title").toUpperCase(),category:fd.get("category"),agentId,reward,rewardCurrency,prizePoolAmountSol:rewardCurrency==="SOL"?reward:null,fundingTxHash,rewardWallet:REWARD_WALLET,deadline:deadlineIso(fd),description:fd.get("description"),rules:createMissionRules(fd),allowAi,proof:createMissionProof(fd)})});await refreshRemoteData();showToast("REWARD POOL FUNDED // MISSION ACTIVE");navigate("/missions");}catch(error){handleFundingError(error,"Transaction cancelled");}finally{done();}}
  if(form.id==="admin-mission-form"){const done=setBusy(form.querySelector("button[type='submit']"),"FUNDING...");try{const rewardCurrency=fd.get("rewardCurrency");const reward=Number(fd.get("reward"));const allowAi=fd.get("allowAi")==="on";showToast("CONFIRM FUNDING IN WALLET");const fundingTxHash=await sendRewardPayment(reward,rewardCurrency,"fund this mission");await api("/admin/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,title:fd.get("title").toUpperCase(),category:fd.get("category"),agentId:fd.get("agentId"),reward,rewardCurrency,fundingTxHash,deadline:deadlineIso(fd),description:fd.get("description"),rules:createMissionRules(fd),allowAi,proof:createMissionProof(fd),featured:fd.get("featured")==="true"})});await refreshRemoteData();liveData.admin=null;showToast("REWARD POOL FUNDED // MISSION ACTIVE");renderAdmin();}catch(error){handleFundingError(error,"Transaction cancelled");}finally{done();}}

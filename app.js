...
function agent(id) {
  const list = liveData.agents || DATA.agents;
  const normalized = id === "neo" ? "neo-agent" : id;
  return list.find((item) => item.id === normalized) || list[0];
}
function agents() { return liveData.agents || DATA.agents; }
...
async function syncWalletProfile(wallet) {
  if (!API_BASE) {
    profileSyncWarning = "Profile details could not be updated online.";
    return null;
  }
  try {
...
async function sendSolPayment(amountSol, label="fund this action") {
  ...
  if (!REWARD_WALLET) throw new Error("Reward wallet is not configured. Set VITE_REWARD_WALLET.");
...
function renderCreate() {
  if(!state.wallet) return app.innerHTML=`${pageTop("AGENT CONSOLE // LOCKED","CREATE MISSION","Only registered agent owners can create missions. Connect your real Solana wallet first.")}<section class="section-shell content-section compact"><button class="button primary" data-open-wallet>CONNECT WALLET</button></section>`;
...
function renderAdmin() {
  ...
  <section class="section-shell admin-grid">
    ...
    <article class="panel"><h3 class="panel-title">INTEGRATIONS</h3><div class="admin-row stacked"><b>CLAWPUMP</b><span>${claw.configured ? "CONFIGURED" : "NOT CONFIGURED"} // ${claw.agentId || "NO AGENT ID"}</span><button class="mini-button quiet" data-admin-claw-test>TEST CONNECTION</button></div><a class="mini-button quiet" href="${routeHref("/developers")}" data-route>DEVELOPER DOCS</a></article>
...
function openBoost(id){ const item=mission(id); modal(`<p class="eyebrow">REWARD SIGNAL // ${item.title}</p><h2>BOOST REWARD</h2><p>Your wallet will send SOL to the protocol reward wallet first. The backend verifies the transaction before the boost is recorded.</p><form id="boost-form" data-id="${id}"><div class="boost-total triple"><span>CURRENT POOL <b>${money(pool(item))}</b></span><span>YOUR BOOST <b data-boost-preview>${sol(0)}</b></span><span>NEW DISPLAYED POOL <b data-boost-total>${money(pool(item))}</b></span></div><label>AMOUNT TO BOOST (SOL)${solField("amountSol","", "required")}</label><p class="joined-note">Reward wallet: ${REWARD_WALLET ? shortAddress(REWARD_WALLET) : "NOT CONFIGURED"}</p><button class="button primary" type="submit">SEND SOL & CONFIRM BOOST</button></form>`); }

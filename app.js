...
function renderDevelopers() {
  app.innerHTML=`${pageTop("AGENT API // LAZARUS READY","DEVELOPERS","Register an agent, get admin approval, fund rewards, and create safe missions programmatically.")}<section class="section-shell prose"><article><h2>WHAT IS THE LAZY AGENT API?</h2><p>The Lazy Agent API lets approved agent owners create funded missions from external agents, campaign tools, or automation systems. It is designed for agent-created quests with real SOL or USDC reward pools.</p></article><article><h2>REGISTER, APPROVE, GENERATE</h2><p>Connect a Solana wallet, register an agent, wait for admin approval, then generate an API key from the agent profile. API keys are shown once, so copy the key before closing the modal.</p></article><article><h2>FUNDING REQUIREMENTS</h2><p>Mission creation requires a prior SOL or USDC transfer to the protocol reward wallet. Submit the transaction signature as <code>fundingTxHash</code>. The backend verifies the transaction, recipient, sender, amount, currency, success status, and duplicate use before creating the mission.</p></article><article><h2>CREATE MISSION</h2><pre><code>curl -X POST "$API_BASE/agent-api/missions" \\
  -H "Authorization: Bearer lp_agent_your_key" \\
  -H "Content-Type: application/json" \\
  -d '{"title":"CREATE A WORLD CUP MEME","category":"World Cup","rewardPool":100,"rewardCurrency":"USDC","fundingTxHash":"5x...","description":"Create an original football meme.","rules":["Keep it original.","Your X post must tag @LazyProtocol."],"proof":"Public X post URL tagging @LazyProtocol"}'</code></pre></article><article><h2>CATEGORIES</h2><p>Supported categories are World Cup, Creative, Predictions, Research, Community, Real World, Agents, Sponsored, and Protocol Agent.</p></article><article><h2>SUBMISSION VERIFICATION</h2><p>Submission missions that use X proof require the submitted post to come from the connected verified X account and tag @LazyProtocol. This prevents impersonation and copied entries.</p></article><article><h2>RATE LIMIT NOTE</h2><p>Keep mission creation deliberate. Production API keys may be rate limited or paused if they spam unsafe, duplicate, or unfunded missions.</p></article><article><h2>SAFETY RULES</h2><p>Agent-generated missions are rejected if they include harmful physical tasks, illegal behavior, harassment, adult content, scams, gambling framing, or deceptive reward claims.</p></article><article><h2>CLAWPUMP</h2><p>ClawPump integration hooks are prepared in the backend. Configure the ClawPump environment variables when the external agent endpoint is ready.</p></article></section>`;
}
function renderAdmin() {
  ...
  <section class="admin-grid">
    ...
    <article class="panel admin-wide" id="admin-funding" data-admin-section="funding"><h3 class="panel-title">FUNDING HISTORY</h3>${(admin.fundingTransactions || []).map((tx)=>`<div class="admin-row stacked"><b>${tx.type} // ${rewardAmount(tx.amount ?? tx.amountSol, tx.currency || "SOL")}</b><span>${tx.missionTitle || tx.missionId || "UNASSIGNED"} // ${shortAddress(tx.fromWallet)} → ${shortAddress(tx.toWallet)} // ${tx.status}</span><a class="mini-button quiet external" href="https://solscan.io/tx/${tx.txHash}" target="_blank" rel="noreferrer">VIEW TX ↗</a></div>`).join("") || `<div class="empty">NO FUNDING TRANSACTIONS YET.</div>`}</article>
...
function openBoost(id){ const item=mission(id); const currency=rewardCurrency(item); modal(`<p class="eyebrow">REWARD SIGNAL // ${item.title}</p><h2>BOOST REWARD</h2><p>Confirm funding from your wallet. The prize pool only increases after the transaction verifies onchain.</p><form id="boost-form" data-id="${id}" data-currency="${currency}"><div class="boost-total triple"><span>CURRENT POOL <b>${rewardLabel(item)}</b></span><span>YOUR BOOST <b data-boost-preview>${rewardAmount(0,currency)}</b></span><span>NEW POOL <b data-boost-total>${rewardLabel(item)}</b></span></div><label>AMOUNT TO BOOST (${currency})${currency==="SOL"?solField("amount","", "required"):moneyField("amount","", "required min=\"0.000001\" step=\"0.000001\"")}</label><button class="button primary" type="submit">CONFIRM FUNDING</button></form>`); }

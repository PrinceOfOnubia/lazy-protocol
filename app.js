...
function openBoost(id){ const item=mission(id); const currency=rewardCurrency(item); modal(`<p class="eyebrow">REWARD SIGNAL // ${item.title}</p><h2>BOOST REWARD</h2><p>Choose SOL or USDC and confirm funding from your wallet. Same-currency boosts increase the displayed mission pool.</p><form id="boost-form" data-id="${id}" data-currency="${currency}"><div class="boost-total triple"><span>CURRENT POOL <b>${rewardLabel(item)}</b></span><span>YOUR BOOST <b data-boost-preview>${rewardAmountHtml(0,currency)}</b></span><span>NEW POOL <b data-boost-total>${boostTotalHtml(item,0,currency)}</b></span></div><label>BOOST CURRENCY<select name="currency" data-payment-currency>${currencyOptions(currency)}</select></label><label data-payment-amount-label>AMOUNT TO BOOST (${currency})${rewardInput("amount",currency,"","required min=\"0.000001\" step=\"0.000001\"")}</label><button class="button primary" type="submit">CONFIRM FUNDING</button></form>`); }

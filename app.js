...
function moneyField(name, value="", attrs="") {
  return `<div class="currency-input"><span>$</span><input ${attrs} name="${name}" type="number" min="0" value="${value}" placeholder="100"></div>`;
}
function solField(name, value="", attrs="") {
  return `<div class="currency-input"><span>◎</span><input ${attrs} name="${name}" type="number" min="0" step="0.000000001" value="${value}" placeholder="1.5"></div>`;
}
...
function renderSubmissions() {
  ...
  app.innerHTML = `${pageTop("PROOF STREAM // LIVE","SUBMISSIONS","Watch mission entries from the Lazy workforce.")}<section class="content-section section-shell compact"><div class="filter-row">${["All","Pending","Approved","Winners","World Cup"].map((item)=>`<button class="filter ${item===submissionFilter?"active":""}" data-submission-filter="${item}">${item.toUpperCase()}</button>`).join("")}</div><div class="submission-grid">${list.length ? list.map(submissionCard).join("") : `<div class="empty">${API_BASE ? "NO SUBMISSIONS MATCH THIS FILTER YET." : "CONNECT THE BACKEND API TO LOAD DATABASE SUBMISSIONS."}</div>`}</div></section>`;
  renderXEmbeds();
}
...
function renderAgentRegister() {
  if(!state.wallet) return app.innerHTML=`${pageTop("AGENT REGISTRY // LOCKED","REGISTER AGENT","Connect a real Solana wallet before registering an agent owner.")}<section class="section-shell content-section compact"><button class="button primary" data-open-wallet>CONNECT WALLET</button></section>`;
  app.innerHTML=`${pageTop("AGENT REGISTRY // OWNER MODE","REGISTER AGENT","Create an agent profile tied to your connected wallet. Admin approval is required before the agent can deploy funded missions or use the agent API.")}<section class="section-shell form-shell"><form id="agent-register-form" class="panel form-grid"><label>OWNER WALLET<input readonly value="${state.wallet}"></label><label>AGENT NAME<input required name="name" placeholder="NEO AGENT"></label><label>HANDLE<input name="handle" placeholder="@neo.agent"></label><label>CATEGORY<select name="category">${categoryOptions("Agents")}</select></label><label>AVATAR URL<input name="avatarUrl" placeholder="https://..."></label><label>WEBSITE<input name="website" placeholder="https://..."></label><label>X HANDLE<input name="xHandle" placeholder="@agent"></label><label class="full-field">BIO<textarea required name="bio" placeholder="What missions will this agent create?"></textarea></label><button class="button primary" type="submit">REGISTER AGENT</button></form></section>`;
}
...
function renderDevelopers() {
  app.innerHTML=`${pageTop("AGENT API // LAZARUS READY","DEVELOPERS","Register an agent, get admin approval, generate an API key, and create safe missions programmatically.")}<section class="section-shell prose"><article><h2>AGENT MISSION API</h2><p>Approved agent owners can generate an API key from their agent profile. Store the key securely. Lazy Protocol only shows it once.</p><pre><code>curl -X POST "$API_BASE/agent-api/missions" \\
  -H "Authorization: Bearer lp_agent_your_key" \\
  -H "Content-Type: application/json" \\
  -d '{"title":"CREATE A WORLD CUP MEME","category":"World Cup","rewardPool":100,"description":"Create an original football meme.","rules":["Keep it original.","Your X post must tag @LazyProtocol."],"proof":"Public X post URL tagging @LazyProtocol"}'</code></pre></article><article><h2>SAFETY RULES</h2><p>Agent-generated missions are rejected if they include harmful physical tasks, illegal behavior, harassment, adult content, scams, or gambling framing.</p></article><article><h2>CLAWPUMP</h2><p>ClawPump integration hooks are prepared in the backend. Configure the ClawPump environment variables when the external agent endpoint is ready.</p></article></section>`;
}
function renderAdmin() {
  ...
  <section class="section-shell admin-dashboard">
  ...
  <section class="section-shell admin-grid">
    <article class="panel"><h3 class="panel-title">CREATE MISSION</h3><form id="admin-mission-form" class="admin-form"><label>TITLE<input required name="title" placeholder="CREATE A WORLD CUP MEME"></label><label>CATEGORY<select name="category">${categoryOptions("World Cup")}</select></label><label>AGENT CREATOR<select required name="agentId">${agentOptions}</select></label><label>REWARD POOL${moneyField("reward","", "required min=\"1\"")}</label><label>DEADLINE<input required type="datetime-local" name="deadline"></label><label>FEATURED<select name="featured"><option value="">NO</option><option value="true">YES</option></select></label><label>DESCRIPTION<textarea required name="description"></textarea></label><label>RULES<textarea required name="rules">${LAZY_X_RULE}</textarea></label><label>PROOF<input required name="proof" value="Public X post URL tagging @LazyProtocol"></label><button class="mini-button primary" type="submit">CREATE</button></form></article>
    <article class="panel"><h3 class="panel-title">LAZARUS GENERATOR</h3><p class="page-copy">Create safe protocol-native missions from Lazarus templates.</p><form id="admin-lazarus-form" class="admin-form"><label>TEMPLATE<select name="type">${lazarusTemplates.map((t)=>`<option value="${t.id}">${t.title}</option>`).join("")}</select></label><label>REWARD POOL${moneyField("rewardPool","100","required min=\"1\"")}</label><label>DEADLINE HOURS<input type="number" min="1" name="deadlineHours" value="48"></label><label>FEATURED<select name="featured"><option value="">NO</option><option value="true">YES</option></select></label><button class="mini-button primary" type="submit">CREATE WITH LAZARUS</button></form></article>
    <article class="panel"><h3 class="panel-title">INTEGRATIONS</h3><div class="admin-row stacked"><b>CLAWPUMP</b><span>${claw.configured ? "CONFIGURED" : "OPTIONAL SETUP NEEDED"} // ${claw.agentId || "NO AGENT ID"}</span><button class="mini-button quiet" data-admin-claw-test>TEST CONNECTION</button></div><a class="mini-button quiet" href="${routeHref("/developers")}" data-route>DEVELOPER DOCS</a></article>
    <article class="panel"><h3 class="panel-title">MISSIONS</h3>${admin.missions.map((m)=>`<div class="admin-row stacked"><b>${m.title}</b><span>${m.category} // ${m.status} // ${money(m.reward)}</span><div class="admin-actions"><button class="mini-button quiet" data-admin-feature="${m.id}">FEATURE</button><button class="mini-button quiet" data-admin-expire="${m.id}">EXPIRE</button><button class="mini-button quiet" data-admin-review="${m.id}">REVIEW</button><button class="mini-button quiet" data-admin-remove="${m.id}">REMOVE</button><button class="mini-button quiet" data-admin-boost="${m.id}">BOOST</button><button class="mini-button quiet" data-admin-extend="${m.id}">EXTEND</button></div></div>`).join("")}</article>
    <article class="panel admin-wide"><div class="section-heading mini"><h3 class="panel-title">SUBMISSIONS</h3><a class="text-link" href="${routeHref("/submissions")}" data-route>PUBLIC VIEW</a></div><div class="admin-filter-row"><select data-admin-status>${["All","PENDING","APPROVED","REJECTED","WINNER","DISQUALIFIED","PAID"].map((s)=>`<option ${s===adminStatusFilter?"selected":""}>${s}</option>`).join("")}</select><select data-admin-category>${categoryOptions(adminCategoryFilter,true)}</select><select data-admin-mission><option ${adminMissionFilter==="All"?"selected":""}>All</option>${admin.missions.map((m)=>`<option value="${m.id}" ${m.id===adminMissionFilter?"selected":""}>${m.title}</option>`).join("")}</select></div>${filteredSubmissions.map((s)=>`<div class="admin-row stacked"><b>${s.title}</b><span>${s.missionTitle} // ${s.status} // ${s.submitterWallet || s.user || ""} // ${s.xHandle ? `@${s.xHandle}` : "NO X"}</span><div class="admin-actions"><button class="mini-button quiet" data-admin-approve="${s.id}">APPROVE</button><button class="mini-button quiet" data-admin-reject="${s.id}">REJECT</button><button class="mini-button quiet" data-admin-winner="${s.id}" ${s.status==="DISQUALIFIED"?"disabled":""}>WINNER</button><button class="mini-button quiet" data-admin-disqualify="${s.id}">DISQUALIFY</button>${["WINNER","PAID"].includes(s.status)?`<button class="mini-button primary" data-admin-pay="${s.id}">PAY USER</button>`:""}</div></div>`).join("") || `<div class="empty">NO SUBMISSIONS MATCH THESE FILTERS.</div>`}</article>
    <article class="panel"><h3 class="panel-title">WINNER PAYOUTS</h3><p class="page-copy">Manual payout mode: send funds from your wallet, then paste transaction hash and mark paid.</p>${winners.map((s)=>`<div class="admin-row stacked"><b>${s.missionTitle}</b><span>${s.payoutStatus || "UNPAID"} // ${s.payoutWallet || s.submitterWallet || ""} // ${s.payoutAmount ? money(s.payoutAmount) : "AMOUNT NEEDED"}</span><button class="mini-button primary" data-admin-pay="${s.id}">${s.payoutStatus==="PAID"?"EDIT PAID":"PAY USER"}</button></div>`).join("") || `<div class="empty">NO WINNERS YET.</div>`}</article>
    <article class="panel"><h3 class="panel-title">USERS</h3><a class="button secondary full-width" href="${routeHref("/admin/users")}" data-route>VIEW ALL USERS</a>${admin.users.slice(0,8).map((u)=>`<div class="admin-row"><b>${u.username || u.wallet}</b><span>${u.status || "ACTIVE"} // ${u.xVerified ? `@${u.xHandle}` : "X NOT VERIFIED"}</span></div>`).join("")}</article>
    <article class="panel"><h3 class="panel-title">AGENTS</h3>${admin.agents.map((a)=>`<div class="admin-row stacked"><b>${a.name}</b><span>${a.ownerWallet ? shortAddress(a.ownerWallet) : "NO OWNER"} // ${a.status || (a.approved ? "APPROVED" : "PENDING")} // ${a.source || "EXTERNAL"} // API ${a.hasApiKey ? "ACTIVE" : "OFF"}</span><div class="admin-actions"><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-status="APPROVED">APPROVE</button><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-status="REJECTED">REJECT</button><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-status="SUSPENDED">SUSPEND</button><button class="mini-button quiet" data-admin-agent="${a.id}" data-agent-featured="true">FEATURE</button><button class="mini-button quiet" data-agent-api-key="${a.id}">${a.hasApiKey ? "ROTATE KEY" : "GENERATE KEY"}</button>${a.hasApiKey ? `<button class="mini-button quiet" data-admin-agent-revoke="${a.id}">REVOKE KEY</button>` : ""}</div></div>`).join("") || `<div class="empty">NO AGENTS.</div>`}</article>
    <article class="panel admin-wide"><h3 class="panel-title">FUNDING HISTORY</h3>${(admin.fundingTransactions || []).map((tx)=>`<div class="admin-row stacked"><b>${tx.type} // ${sol(tx.amountSol)}</b><span>${tx.missionTitle || tx.missionId || "UNASSIGNED"} // ${shortAddress(tx.fromWallet)} → ${shortAddress(tx.toWallet)} // ${tx.status}</span><a class="text-link" href="https://solscan.io/tx/${tx.txHash}" target="_blank" rel="noreferrer">VIEW TX →</a></div>`).join("") || `<div class="empty">NO FUNDING TRANSACTIONS YET.</div>`}</article>
    <article class="panel"><h3 class="panel-title">EXPORTS</h3><div class="admin-actions"><button class="mini-button quiet" data-export="users">USERS CSV</button><button class="mini-button quiet" data-export="submissions">SUBMISSIONS CSV</button><button class="mini-button quiet" data-export="winners">WINNERS CSV</button></div></article>
  </section>`;
}
...
function openBoost(id){ const item=mission(id); modal(`<p class="eyebrow">REWARD SIGNAL // ${item.title}</p><h2>BOOST REWARD</h2><p>Boosts increase the mission reward pool record and appear in admin payout operations.</p><form id="boost-form" data-id="${id}"><div class="boost-total triple"><span>CURRENT POOL <b>${money(pool(item))}</b></span><span>YOUR BOOST <b data-boost-preview>${money(0)}</b></span><span>NEW POOL <b data-boost-total>${money(pool(item))}</b></span></div><label>AMOUNT TO BOOST${moneyField("amount","", "required min=\"1\"")}</label><button class="button primary" type="submit">CONFIRM BOOST</button></form>`); }
...
function openAdminPay(id){ const s=(liveData.admin?.submissions||[]).find((item)=>item.id===id); if(!s)return; modal(`<p class="eyebrow">MANUAL PAYOUT MODE</p><h2>PAY USER</h2><p>Send funds from your wallet, then paste the transaction hash and mark paid.</p><form id="admin-pay-form" data-id="${id}"><label>WINNER WALLET<input required name="payoutWallet" value="${s.payoutWallet || s.submitterWallet || ""}"></label><label>PAYOUT AMOUNT${moneyField("payoutAmount",s.payoutAmount||"","required")}</label><label>TRANSACTION HASH<input required name="payoutTxHash" value="${s.payoutTxHash || ""}" placeholder="Paste tx hash"></label><label>PAYOUT NOTE<textarea name="payoutNote">${s.payoutNote || ""}</textarea></label><div class="action-row"><button class="button secondary" type="button" data-copy="${s.payoutWallet || s.submitterWallet || ""}">COPY WALLET</button><button class="button secondary" type="button" data-copy="${s.payoutAmount || ""}">COPY AMOUNT</button></div><button class="button primary" type="submit">MARK PAID</button></form>`); }
function openAdminMissionModal(id,type){ const labels={boost:["BOOST REWARD","admin-boost-form","rewardBoost","Amount to add"],extend:["EXTEND DEADLINE","admin-extend-form","deadline","New deadline"],remove:["REMOVE MISSION","admin-remove-form","reason","Removal reason"]}; const [title,form,field,label]=labels[type]; const input=type==="boost"?moneyField(field,"","required min=\"1\""):type==="extend"?`<input required type="datetime-local" name="${field}">`:`<textarea required name="${field}"></textarea>`; modal(`<p class="eyebrow">ADMIN MISSION OPS</p><h2>${title}</h2><form id="${form}" data-id="${id}"><label>${label}${input}</label><button class="button primary" type="submit">CONFIRM</button></form>`); }
...
document.addEventListener("click",(event)=>{
  ...
  const clawTest=event.target.closest("[data-admin-claw-test]"); if(clawTest){api("/admin/integrations/clawpump/test",{method:"POST",body:JSON.stringify({wallet:state.wallet})}).then((payload)=>showToast(payload.clawpump?.message || "CLAWPUMP TEST COMPLETE")).catch((error)=>showToast(error.message));return;}
  ...
  const copy=event.target.closest("[data-copy]"); if(copy){navigator.clipboard?.writeText(copy.dataset.copy||"");showToast("COPIED");return;}
...
document.addEventListener("input",(event)=>{if(event.target.name==="amount"&&document.querySelector("[data-boost-total]")){const item=mission(event.target.closest("form").dataset.id);const boost=Number(event.target.value||0);document.querySelector("[data-boost-preview]").textContent=money(boost);document.querySelector("[data-boost-total]").textContent=money(pool(item)+boost);}if(event.target.id==="agent-search"){agentQuery=event.target.value;renderAgents();document.querySelector("#agent-search")?.focus();}if(event.target.matches("[data-admin-user-search]")){adminUserSearch=event.target.value;liveData.adminUsers=null;clearTimeout(window.__adminSearch);window.__adminSearch=setTimeout(()=>renderAdminUsers(),250);}});
document.addEventListener("submit", async (event)=>{
  event.preventDefault(); const form=event.target; const fd=new FormData(form);
  if(form.id==="boost-form"){try{const id=form.dataset.id;const amount=Number(fd.get("amount"));await api(`/missions/${id}/boost`,{method:"POST",body:JSON.stringify({wallet:state.wallet,amount})});state.boosts[id]=Number(state.boosts[id]||0)+amount;save();closeModal();await refreshRemoteData();showToast("BOOST RECORDED");}catch(error){showToast(error.message);}}
  ...
  if(form.id==="admin-pay-form"){try{await api(`/admin/submissions/${form.dataset.id}/mark-paid`,{method:"POST",body:JSON.stringify({wallet:state.wallet,payoutWallet:fd.get("payoutWallet"),payoutAmount:Number(fd.get("payoutAmount")),payoutTxHash:fd.get("payoutTxHash"),payoutNote:fd.get("payoutNote")})});closeModal();liveData.admin=null;renderAdmin();showToast("WINNER MARKED PAID");}catch(error){showToast(error.message);}}
  if(form.id==="admin-boost-form"){try{await api(`/admin/missions/${form.dataset.id}`,{method:"PATCH",body:JSON.stringify({wallet:state.wallet,rewardBoost:Number(fd.get("rewardBoost"))})});closeModal();liveData.admin=null;await refreshRemoteData();renderAdmin();showToast("REWARD POOL UPDATED");}catch(error){showToast(error.message);}}
  if(form.id==="admin-extend-form"){try{await api(`/admin/missions/${form.dataset.id}`,{method:"PATCH",body:JSON.stringify({wallet:state.wallet,deadline:new Date(fd.get("deadline")).toISOString()})});closeModal();liveData.admin=null;renderAdmin();showToast("DEADLINE EXTENDED");}catch(error){showToast(error.message);}}
  ...
  if(form.id==="create-form"){try{const prizePoolAmountSol=Number(fd.get("prizePoolSol"));showToast("OPEN WALLET TO FUND MISSION");const fundingTxHash=await sendSolPayment(prizePoolAmountSol,"fund this mission");await api("/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,title:fd.get("title").toUpperCase(),category:fd.get("category"),agentId:fd.get("agentId"),reward:prizePoolAmountSol,prizePoolAmountSol,fundingTxHash,rewardWallet:REWARD_WALLET,deadline:new Date(fd.get("deadline")).toISOString(),description:fd.get("description"),rules:ensureRules(fd.get("rules").split("\n").filter(Boolean)),proof:fd.get("proof")})});await refreshRemoteData();showToast("MISSION FUNDED AND DEPLOYED");navigate("/missions");}catch(error){showToast(error.message);}}
  if(form.id==="admin-mission-form"){try{await api("/admin/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,title:fd.get("title").toUpperCase(),category:fd.get("category"),agentId:fd.get("agentId"),reward:Number(fd.get("reward")),deadline:new Date(fd.get("deadline")).toISOString(),description:fd.get("description"),rules:ensureRules(fd.get("rules").split("\n").filter(Boolean)),proof:fd.get("proof"),featured:fd.get("featured")==="true"})});await refreshRemoteData();liveData.admin=null;showToast("MISSION DEPLOYED");renderAdmin();}catch(error){showToast(error.message);}}
  if(form.id==="admin-lazarus-form"){try{await api("/admin/lazarus/create-mission",{method:"POST",body:JSON.stringify({wallet:state.wallet,type:fd.get("type"),rewardPool:Number(fd.get("rewardPool")),deadlineHours:Number(fd.get("deadlineHours")),featured:fd.get("featured")==="true"})});await refreshRemoteData();liveData.admin=null;showToast("LAZARUS MISSION CREATED");renderAdmin();}catch(error){showToast(error.message);}}
});

...
function rewardLabel(item) { return rewardAmount(pool(item), rewardCurrency(item)); }
...
function renderCreate() {
  ...
  app.innerHTML=`${pageTop("AGENT CONSOLE // NEW","CREATE MISSION","Deploy a funded mission for an approved agent. SOL pools can be funded onchain now; USDC pools are tracked for manual payout operations.")}<section class="section-shell form-shell"><form id="create-form" class="panel form-grid" data-reward-form><label>MISSION TITLE<input required name="title" placeholder="CREATE A MATCH-DAY MEME"></label><label>CATEGORY<select name="category">${categoryOptions("World Cup")}</select></label><label>AGENT CREATOR<select name="agentId">${owned.map((a)=>`<option value="${a.id}">${a.name}</option>`).join("")}</select></label><label>REWARD CURRENCY<select name="rewardCurrency" data-reward-currency>${currencyOptions("USDC")}</select></label><label data-reward-amount-label>REWARD POOL${moneyField("reward","", "required min=\"0.000000001\" step=\"0.000000001\"")}</label><label>DEADLINE<input required type="datetime-local" name="deadline"></label><label class="full-field">DESCRIPTION<textarea required name="description" placeholder="Describe the mission clearly."></textarea></label><label class="full-field">RULES<textarea required name="rules">${LAZY_X_RULE}</textarea></label><label class="full-field">PROOF REQUIREMENT<input required name="proof" placeholder="Public X post URL tagging @LazyProtocol"></label><p class="full-field joined-note" data-funding-note>USDC rewards are tracked for manual payout. Switch to SOL for onchain funding verification.</p><button class="button primary" type="submit">DEPLOY MISSION</button></form></section>`;
}
...
function renderAdmin() {
  ...
  <section class="admin-grid">
    ...
    <article class="panel admin-wide" id="admin-funding" data-admin-section="funding"><h3 class="panel-title">FUNDING HISTORY</h3>${(admin.fundingTransactions || []).map((tx)=>`<div class="admin-row stacked"><b>${tx.type} // ${sol(tx.amountSol)}</b><span>${tx.missionTitle || tx.missionId || "UNASSIGNED"} // ${shortAddress(tx.fromWallet)} → ${shortAddress(tx.toWallet)} // ${tx.status}</span><a class="mini-button quiet external" href="https://solscan.io/tx/${tx.txHash}" target="_blank" rel="noreferrer">VIEW TX ↗</a></div>`).join("") || `<div class="empty">NO FUNDING TRANSACTIONS YET.</div>`}</article>
...
function openBoost(id){ const item=mission(id); const currency=rewardCurrency(item); modal(`<p class="eyebrow">REWARD SIGNAL // ${item.title}</p><h2>BOOST REWARD</h2><p>Boosts increase the mission reward pool record and appear in admin payout operations.</p><form id="boost-form" data-id="${id}" data-currency="${currency}"><div class="boost-total triple"><span>CURRENT POOL <b>${rewardLabel(item)}</b></span><span>YOUR BOOST <b data-boost-preview>${rewardAmount(0,currency)}</b></span><span>NEW POOL <b data-boost-total>${rewardLabel(item)}</b></span></div><label>AMOUNT TO BOOST (${currency})${currency==="SOL"?solField("amount","", "required"):moneyField("amount","", "required min=\"1\"")}</label><button class="button primary" type="submit">CONFIRM BOOST</button></form>`); }
...
document.addEventListener("change",(event)=>{
  ...
  if(event.target.matches("[data-reward-currency]")){const form=event.target.closest("[data-reward-form]");const label=form?.querySelector("[data-reward-amount-label]");const note=form?.querySelector("[data-funding-note]");const fieldName=form?.id==="admin-lazarus-form"?"rewardPool":"reward";const value=fieldName==="rewardPool"?"100":"";if(label)label.innerHTML=`REWARD POOL${rewardInput(fieldName,event.target.value,value,"required min=\"0.000000001\" step=\"0.000000001\"")}`;if(note)note.textContent=event.target.value==="SOL"?(REWARD_WALLET?`SOL funding will be sent to protocol wallet ${shortAddress(REWARD_WALLET)} before deployment.`:"SOL funding requires a protocol reward wallet."):"USDC rewards are tracked for manual payout. Switch to SOL for onchain funding verification.";return;}
...
document.addEventListener("submit", async (event)=>{
  ...
  if(form.id==="admin-boost-form"){try{await api(`/admin/missions/${form.dataset.id}`,{method:"PATCH",body:JSON.stringify({wallet:state.wallet,rewardBoost:Number(fd.get("rewardBoost"))})});closeModal();liveData.admin=null;await refreshRemoteData();renderAdmin();showToast(`REWARD POOL UPDATED (${form.dataset.currency || "USDC"})`);}catch(error){showToast(error.message);}}
  ...
  if(form.id==="create-form"){try{const rewardCurrency=fd.get("rewardCurrency");const reward=Number(fd.get("reward"));let fundingTxHash="";if(rewardCurrency==="SOL"){showToast("OPEN WALLET TO FUND SOL MISSION");fundingTxHash=await sendSolPayment(reward,"fund this mission");}await api("/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,title:fd.get("title").toUpperCase(),category:fd.get("category"),agentId:fd.get("agentId"),reward,rewardCurrency,prizePoolAmountSol:rewardCurrency==="SOL"?reward:null,fundingTxHash,rewardWallet:REWARD_WALLET,deadline:new Date(fd.get("deadline")).toISOString(),description:fd.get("description"),rules:ensureRules(fd.get("rules").split("\n").filter(Boolean)),proof:fd.get("proof")})});await refreshRemoteData();showToast(rewardCurrency==="SOL"?"MISSION FUNDED AND DEPLOYED":"MISSION DEPLOYED");navigate("/missions");}catch(error){showToast(error.message);}}
  if(form.id==="admin-mission-form"){try{await api("/admin/missions",{method:"POST",body:JSON.stringify({wallet:state.wallet,title:fd.get("title").toUpperCase(),category:fd.get("category"),agentId:fd.get("agentId"),reward:Number(fd.get("reward")),rewardCurrency:fd.get("rewardCurrency"),deadline:new Date(fd.get("deadline")).toISOString(),description:fd.get("description"),rules:ensureRules(fd.get("rules").split("\n").filter(Boolean)),proof:fd.get("proof"),featured:fd.get("featured")==="true"})});await refreshRemoteData();liveData.admin=null;showToast("MISSION DEPLOYED");renderAdmin();}catch(error){showToast(error.message);}}
  if(form.id==="admin-lazarus-form"){try{await api("/admin/lazarus/create-mission",{method:"POST",body:JSON.stringify({wallet:state.wallet,type:fd.get("type"),rewardPool:Number(fd.get("rewardPool")),rewardCurrency:fd.get("rewardCurrency"),description:fd.get("description"),deadlineHours:Number(fd.get("deadlineHours")),featured:fd.get("featured")==="true"})});await refreshRemoteData();liveData.admin=null;showToast("LAZARUS MISSION CREATED");renderAdmin();}catch(error){showToast(error.message);}}
});

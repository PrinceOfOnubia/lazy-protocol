...
function heroSlideMarkup() {
  ...
  const slides = [
    `<div class="hero-slide active" data-hero-panel="0"><p class="eyebrow">LAZY PROTOCOL v0.2 <span>//</span> HUMAN LAYER ONLINE</p><h1 class="hero-core-title"><span>TURN HUMAN ATTENTION</span><span>INTO AN ONCHAIN</span><span>WORKFORCE</span></h1><p class="hero-copy lead">Agents create opportunities. Humans create outcomes. Rewards align incentives.</p><p class="hero-small-line">Not humans versus agents. Humans and agents working together.</p><div class="hero-stats"><div><small>OPEN MISSIONS:</small><strong>${missions().filter((m)=>statusFor(m)!=="Expired").length}</strong></div><div><small>REWARDS PAID:</small><strong>${money(totalRewardsPaid())}</strong></div><div><small>HUMANS JOINED:</small><strong>${totalHumansJoined().toLocaleString()}</strong></div></div><div class="hero-actions three"><a class="button primary" href="${routeHref("/missions")}" data-route>EXPLORE MISSIONS</a><a class="button secondary" href="${routeHref("/world-cup")}" data-route>WORLD CUP FANTASY</a><a class="button tertiary" href="${routeHref("/missions/create")}" data-route>CREATE MISSION</a></div></div>`,
    `<div class="hero-slide hero-slide-experiment" data-hero-panel="1"><p class="eyebrow">PUBLIC EXPERIMENT <span>//</span> HUMAN + AGENT COORDINATION</p><h1 class="hero-title-alt hero-title-experiment">ARE HUMANS<br><span>REALLY LAZY?</span></h1><p class="hero-copy lead">Lazy Protocol tests a different future in public: humans and agents working together through missions, proof, and rewards.</p><div class="hero-experiment-card"><small>LIVE COORDINATION TEST</small><strong>HUMANS + AGENTS</strong><span>Every mission is another data point.</span></div><div class="hero-actions"><a class="button primary" href="${routeHref("/about#experiment")}" data-route>JOIN THE EXPERIMENT</a></div></div>`,
    `<div class="hero-slide" data-hero-panel="2"><p class="eyebrow">SEASON 01 <span>//</span> FEATURED CAMPAIGN</p><h1 class="hero-title-alt">WORLD CUP<br><span>FANTASY</span></h1><p class="hero-copy lead">Predictions, missions, fan content, and reward pools powered by agents and football communities.</p><div class="hero-bounty"><small>FEATURED REWARD POOL</small><strong>${rewardLabel(topMission)}</strong><span data-countdown="${topMission.id}">${countdown(topMission)}</span></div><div class="hero-actions"><a class="button primary" href="${routeHref("/world-cup")}" data-route>ENTER FANTASY</a></div></div>`
  ];

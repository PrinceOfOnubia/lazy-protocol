...
function renderAdmin() {
  ...
  <section class="admin-grid">
    ...
    <article class="panel admin-wide" id="admin-funding" data-admin-section="funding"><h3 class="panel-title">FUNDING HISTORY</h3>${(admin.fundingTransactions || []).map((tx)=>`<div class="admin-row stacked"><b>${tx.type} // ${rewardAmountHtml(tx.amount ?? tx.amountSol, tx.currency || "SOL")}</b><span>${tx.missionTitle || tx.missionId || "UNASSIGNED"} // ${shortAddress(tx.fromWallet)} → ${shortAddress(tx.toWallet)} // ${tx.status}</span><a class="mini-button quiet external" href="https://solscan.io/tx/${tx.txHash}" target="_blank" rel="noreferrer">VIEW TX ↗</a></div>`).join("") || `<div class="empty">NO FUNDING TRANSACTIONS YET.</div>`}</article>
